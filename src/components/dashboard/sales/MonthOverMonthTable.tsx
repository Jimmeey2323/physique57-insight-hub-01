...
export const MonthOverMonthTable: React.FC<MonthOverMonthTableProps> = ({
  data,
  onRowClick,
  ...
  const processedData = useMemo(() => {
    ...
    const products = Object.entries(productGroups).map(([product, items]) => {
      ...
      const total = getMetricValue(items, selectedMetric);
  ...
  const totalsRow = useMemo(() => {
    ...
    return {
      product: 'TOTAL',
      total: getMetricValue(data, selectedMetric),
